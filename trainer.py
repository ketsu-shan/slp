...
class Trainer(BaseTrainer):
    ...
    def compute_loss(self,pred_rotmat, pred_betas, pred_camera,
                          batch_size, gt_keypoints_2d_orig,
                          opt_joint_loss, opt_vertices, opt_joints,
                          opt_pose, opt_betas, opt_cam_t,
                          dataset_name, indices, rot_angle,
                          is_flipped, has_smpl, gt_vertices,
                          gt_cam_t, gt_model_joints, gt_pose,
                          gt_betas, gt_keypoints_2d, gt_joints,
                          has_pose_3d, loss_id):
        ...
        if self.options.run_smplify:
            ...
            self.fits_dict[(dataset_name, indices.cpu(), rot_angle.cpu(), is_flipped.cpu(), update.cpu())] = (
        ...
        output = {'pred_vertices'+loss_id: pred_vertices.detach(),
                  'opt_vertices'+loss_id: opt_vertices,
                  'pred_cam_t'+loss_id: pred_cam_t.detach(),
                  'opt_cam_t'+loss_id: opt_cam_t}

        losses = {'loss'+loss_id: loss.detach().item(),
                  'loss_keypoints'+loss_id: loss_keypoints.detach().item(),
                  'loss_keypoints_3d'+loss_id: loss_keypoints_3d.detach().item(),
                  'loss_regr_pose'+loss_id: loss_regr_pose.detach().item(),
                  'loss_regr_betas'+loss_id: loss_regr_betas.detach().item(),
                  'loss_shape'+loss_id: loss_shape.detach().item()}
    ...
    def train_step(self, input_batch):
        ...
        #depths = input_batch['depth']  # input depth
        ...
        opt_joint_loss = self.smplify.get_fitting_loss(opt_pose, opt_betas, opt_cam_t,
                                                       0.5 * self.options.img_res * torch.ones(batch_size, 2,
                                                                                               device=self.device),
        ...
        if 1:
            ...
            output, losses, loss = self.compute_loss(pred_rotmat, pred_betas, pred_camera,
                              batch_size, gt_keypoints_2d_orig,
                              opt_joint_loss, opt_vertices, opt_joints,
                              opt_pose, opt_betas, opt_cam_t,
                              dataset_name, indices, rot_angle,
            ...
            # output_1, losses_1, loss_1 = self.compute_loss(pred_rotmat_1, pred_betas_1, pred_camera_1,
            #                                    batch_size, gt_keypoints_2d_orig,
            #                                    opt_joint_loss, opt_vertices, opt_joints,
            #                                    opt_pose, opt_betas, opt_cam_t,
            #                                    dataset_name, indices, rot_angle,
            ...
            #                                    gt_cam_t, gt_model_joints, gt_pose,
            #                                    gt_betas, gt_keypoints_2d, gt_joints,
            #                                    has_pose_3d, loss_id='1')
            #
            # output_2, losses_2, loss_2 = self.compute_loss(pred_rotmat_2, pred_betas_2, pred_camera_2,
            #                                        batch_size, gt_keypoints_2d_orig,
            #                                        opt_joint_loss, opt_vertices, opt_joints,
            #                                        opt_pose, opt_betas, opt_cam_t,
            #                                        dataset_name, indices, rot_angle,
            ...
            #                                        gt_cam_t, gt_model_joints, gt_pose,
            #                                        gt_betas, gt_keypoints_2d, gt_joints,
            #                                        has_pose_3d, loss_id='2')

            # output.update(output_1)
            # losses.update(losses_1)
            # output.update(output_2)
            # losses.update(losses_2)
